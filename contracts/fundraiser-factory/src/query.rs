...
impl<'a> FundraiserFactoryContract<'a> {
    pub fn query(&self, deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
        match msg {
            QueryMsg::GetFundraisers { limit, offset } => {
                to_binary(&self.get_fundraisers(deps, limit, offset)?)
            }
        }
    }
}
    ...
    fn get_fundraisers(
        ...
        if offset as usize > fundraisers.addrs.len() {
            return Err(StdError::generic_err(
                "offset must be smaller than num of fundraisers",
            ));
        };

        let mut size = fundraisers.addrs.len() as u64 - offset;
        size = if size > limit { limit } else { size };

        size = if size > MAX_LIMIT { MAX_LIMIT } else { size };
